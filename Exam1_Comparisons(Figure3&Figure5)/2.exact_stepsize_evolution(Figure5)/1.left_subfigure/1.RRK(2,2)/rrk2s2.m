function [exact_stepsize,tmesh]=rrk2s2(tau)
...
while (tn<(T-tau))
    ...
    %%%% 2
    Vmid_t=Vn_t+0.5*tau*Fn1(1:N);  Umid_t=Un_t+0.5*tau*Fn1(N+1:2*N);
    Umid=real(W_hat*Umid_t);  Fn2=A*[Vmid_t;Umid_t]+[-W*func_f(Umid);occ];
    %%%% next
    Vn1_t=Vn_t+tau*Fn2(1:N);  Un1_t=Un_t+tau*Fn2(N+1:2*N);
...
Umid=real(W_hat*Umid_t);  Fn2=A*[Vmid_t;Umid_t]+[-W*func_f(Umid);occ];
%%%% next
Vn_t=Vn_t+tau*Fn2(1:N);  Un_t=Un_t+tau*Fn2(N+1:2*N);  tn=tn+tau;
