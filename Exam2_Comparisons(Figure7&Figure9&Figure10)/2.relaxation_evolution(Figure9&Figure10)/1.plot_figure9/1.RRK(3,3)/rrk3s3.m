function [GAMMA,tmesh]=rrk3s3(tau)
...
while (tn<(T-tau))
    ...
    %%%% 2
    Vmid=Vn+(1/3)*tau*Fn1(1:N^2);  Umid=Un+(1/3)*tau*Fn1(N^2+1:2*N^2);
    Fn2=A*[Vmid;Umid]+[-func_f(Umid);occ];
    %%%% 3
    Vmid=Vn+(2/3)*tau*Fn2(1:N^2);  Umid=Un+(2/3)*tau*Fn2(N^2+1:2*N^2);
    Fn3=A*[Vmid;Umid]+[-func_f(Umid);occ];
    %%%% next
    Vn1=Vn+(1/4)*tau*Fn1(1:N^2)+(3/4)*tau*Fn3(1:N^2);  Un1=Un+(1/4)*tau*Fn1(N^2+1:2*N^2)+(3/4)*tau*Fn3(N^2+1:2*N^2);
...
Fn3=A*[Vmid;Umid]+[-func_f(Umid);occ];
%%%% next
Vn=Vn+(1/4)*tau*Fn1(1:N^2)+(3/4)*tau*Fn3(1:N^2);  Un=Un+(1/4)*tau*Fn1(N^2+1:2*N^2)+(3/4)*tau*Fn3(N^2+1:2*N^2);  tn=tn+tau;