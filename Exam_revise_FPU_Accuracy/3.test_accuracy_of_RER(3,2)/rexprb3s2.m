function [yn,xn,gamma_average]=rexprb3s2(tau)
T=1;  m=3;  w=50;  om=zeros(m,m);  Im=eye(m);  o2m=zeros(2*m,2*m);  I2m=eye(2*m);  
AA=[om om; om w^2*Im];  A=[o2m -AA; I2m o2m];  d=4*m;  oc=zeros(d,1);  occ=zeros(d,1);
...
while (tn<(T-tau))
    ...
    oo=zeros(3,d);  II=[0 1 0;0 0 1;0 0 0];  Matrix=tau*[Jn (2/tau/tau)*Dn2 oc F; oo II];
