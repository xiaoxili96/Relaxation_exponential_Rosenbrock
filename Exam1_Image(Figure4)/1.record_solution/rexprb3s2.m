tau=1/150;
T=15;  N=40;  h=2*pi/N;  xmesh=0:h:2*pi-h;  freq=-N/2:N/2-1;  area=2*pi;
...
tn=0;  Vn=zeros(d,1);  Un=(1+cos(xmesh))';  Vn_t=W*Vn;  Un_t=W*Un;  tmesh=[];  U_save=[];
while (tn<(T-tau))
    ...
    energy_old=func_of_energy(Vn_t,Un_t,W_hat,K,area,h,func_F);  tmesh=[tmesh tn];  U_save=[U_save Un];
...
Un=real(W_hat*Un_t);  U_save=[U_save Un];  tmesh=[tmesh tn];

if ( (T-tn)<=0 )
    U_save=U_save(:,1:end-1);  tmesh=tmesh(1,1:end-1);  
